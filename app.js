...
// Set email address with strong obfuscation to prevent scraping
function setEmailAddress() {
    const mailElement = document.getElementById('mail');
    if (!mailElement) return;

    // Сложная обфускация для защиты от парсеров
    const emailParts = [
        ['\u0061\u0064', '\u006d\u0069\u006e'].join(''),
    ...
    // Делаем видимый текст для пользователя
    mailElement.textContent = emailParts.join('');
    
    // Используем JavaScript для обработки клика вместо прямой ссылки
